    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "Gabi Chatbot disable by {}.".format(mention_html(user.id, user.first_name)),
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "Gabi Chatbot enable by {}.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML,
            )
...
def chatbot(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        ...
        kukiurl = requests.get(f"http://api.brainshop.ai/get?bid=176809&key=lbMN8CXTGzhn1NKG&uid=[user]&msg={Message}")
...
def list_all_chats(update: Update, context: CallbackContext):
    ...
    text = "<b>Gabi Chatbot-Enabled Chats</b>\n"
    for chat in chats:
        try:
